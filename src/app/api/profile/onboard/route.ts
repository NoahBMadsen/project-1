import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { sql } from "@/db";

export async function POST(request: Request) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();

  if (!user) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const body = await request.json();
  const { username, display_preference } = body;

  const validPrefs = ["anonymous", "username", "real_name"];
  if (!validPrefs.includes(display_preference)) {
    return NextResponse.json({ error: "Invalid display_preference" }, { status: 400 });
  }

  if (display_preference === "username" && !username?.trim()) {
    return NextResponse.json({ error: "Username required" }, { status: 400 });
  }

  await sql`
    INSERT INTO users (id, email, display_name, username, display_preference, avatar_url, onboarded_at)
    VALUES (
      ${user.id},
      ${user.email ?? ""},
      ${user.user_metadata?.full_name ?? user.user_metadata?.name ?? null},
      ${username?.trim() ?? null},
      ${display_preference},
      ${user.user_metadata?.avatar_url ?? null},
      NOW()
    )
    ON CONFLICT (id) DO UPDATE SET
      display_name = COALESCE(EXCLUDED.display_name, users.display_name),
      username = COALESCE(EXCLUDED.username, users.username),
      display_preference = EXCLUDED.display_preference,
      avatar_url = COALESCE(EXCLUDED.avatar_url, users.avatar_url),
      onboarded_at = COALESCE(users.onboarded_at, NOW()),
      updated_at = NOW()
  `;

  return NextResponse.json({ success: true });
}
